...
namespace ScooterBear.GTD.Application.UserLabel
{
    ...
    public class Label
    {
        ...
        public Label(string id, string name, string userId, int count, int countOverDue,
            DateTime dateCreated, int versionNumber = 0, bool isDeleted = false)
        {
            ...
            if (dateCreated == DateTime.MaxValue || dateCreated == DateTime.MinValue)
                throw new ArgumentException("DateCreated for Label must be valid.", nameof(dateCreated));
